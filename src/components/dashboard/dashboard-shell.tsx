...
const COLLAPSED_STORAGE_KEY = "student-sidebar-collapsed";
...
export function DashboardShell({ user, initialCollapsed, children }: Props) {
  ...
  return (
    <div className="relative flex min-h-screen bg-[color:var(--color-surface-app)] text-zinc-900 dark:text-zinc-100">
      ...
      {/* Floating collapse toggle — anchored to the seam corner where the
          topbar's bottom edge meets the sidebar's right border. The button
          straddles both surfaces (half-overlap) and slides as the sidebar
          width animates. Desktop-only. */}
      <button
        type="button"
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        ...
        >
          <SheetTitle className="sr-only">Menu peserta didik</SheetTitle>
          <StudentSidebar
            variant="drawer"
            collapsed={false}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <DashboardTopbar user={user} onOpenMobileNav={() => setMobileOpen(true)} />
