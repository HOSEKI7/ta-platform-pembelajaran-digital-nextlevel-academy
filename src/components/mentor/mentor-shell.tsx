...
import { MentorTopbar } from "./mentor-topbar";

const COLLAPSED_STORAGE_KEY = "mentor-sidebar-collapsed";
...
export function MentorShell({ user, context, initialCollapsed, children }: Props) {
  ...
  return (
    <div className="relative flex min-h-screen bg-[color:var(--color-surface-app)] text-zinc-900 dark:text-zinc-100">
      ...
      <div className="sticky top-0 hidden h-screen self-start md:block">
        <MentorSidebar collapsed={collapsed} />
      </div>
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        ...
        >
          <SheetTitle className="sr-only">Menu mentor</SheetTitle>
          <MentorSidebar
            variant="drawer"
            collapsed={false}
