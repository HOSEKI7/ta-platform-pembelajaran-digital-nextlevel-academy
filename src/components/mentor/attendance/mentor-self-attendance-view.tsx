...
import {
  useMentorAttendanceMonthQuery,
  useMentorCheckInMutation,
} from "@/hooks/use-mentor-attendance";
import type {
...
import { AttendanceCalendar } from "@/components/internship/attendance/attendance-calendar";
import { AttendanceGuideCard } from "@/components/internship/attendance/attendance-guide-card";
import { CheckInCard } from "@/components/internship/attendance/check-in-card";
...
/**
 * Client composer for `/mentor/attendance` — the mentor's OWN attendance. Mirrors
 * the Peserta-Magang attendance view: the calendar reads real data via TanStack
 * Query (month navigation refetches, clamped to the period); Check-In is a
 * server-gated mutation that invalidates the grid and refreshes server props on
 * success. Reuses the internship attendance presentational components.
 */
export function MentorSelfAttendanceView({
  serverNowISO,
  window,
  period,
  initialYear,
  ...
  const monthQuery = useMentorAttendanceMonthQuery(
    displayed.year,
    displayed.month,
  ...
  return (
    <StudentPageContainer>
      <PageHeader
        ...
        title="Rekam Jejak"
        accent="Kehadiran"
        description="Check-in setiap hari kerja dalam jendela waktu yang ditentukan, dan pantau riwayat kehadiranmu sepanjang program."
      />
