...
import { MentorClassChip } from "./mentor-class-chip";
import { MentorNotificationsButton } from "./mentor-notifications-button";
...
export function MentorTopbar({ user, context, onOpenMobileNav }: Props) {
  return (
    <header
      className={cn(
    ...
    >
      ...
      {/* Right cluster — three semantic groups, two hairline separators.
          Group 1: identity context (Kelas bimbingan) — the mentor counterpart
          to the student gamification LevelChip.
          Group 2: workspace controls (Theme + Notifications).
          Group 3: identity (Profile). */}
      <div className="flex items-center gap-3 sm:gap-4">
        <MentorClassChip context={context} />

        <Separator />

        <div className="flex items-center gap-2.5">
          <ThemeToggle />
          <MentorNotificationsButton />
        </div>
