...
// SVG donut geometry (matches the dashboard ClassAttendanceCard).
const RADIUS = 54;
const STROKE = 14;
const CIRCUMFERENCE = 2 * Math.PI * RADIUS;

const WINDOW_BADGE: Record<MentorWindowState, { label: string; cls: string }> = {
  OPEN: {
...
export function AttendanceSummaryCard({ day }: Props) {
  const { kind, summary, windowState, holidayLabel } = day;
  const isWorking = kind === "WORKING";
  const { present, belum, tidakHadir, total } = summary;

  const ratePct = isWorking && total > 0 ? Math.round((present / total) * 100) : 0;
      ...
      <div className="flex flex-col items-center gap-5 sm:flex-row sm:gap-6">
        ...
        <div className="relative grid shrink-0 place-items-center">
          <svg
            ...
            aria-label={
              isWorking
                ? `${present} dari ${total} peserta hadir`
                : "Tidak ada absensi pada hari ini"
            }
          >
            <circle
              cx={74}
          ...
          <div className="absolute inset-0 grid place-items-center text-center">
            {isWorking ? (
              ...
              <CalendarOff
                className="size-7 text-zinc-300 dark:text-zinc-600"
                strokeWidth={1.8}
              />
            )}
          </div>
        ...
        <div className="flex w-full flex-col gap-2.5">
          {isWorking ? (
            ...
            <div className="rounded-2xl bg-zinc-50 px-4 py-4 text-center sm:text-left dark:bg-white/[0.03]">
              <p className="font-heading text-base font-bold text-zinc-700 dark:text-zinc-200">
                {kind === "LUAR_PERIODE" ? "Di luar periode magang" : "Hari libur"}
              </p>
              <p className="mt-1 text-sm leading-relaxed text-zinc-500 dark:text-zinc-400">
                {kind === "LUAR_PERIODE"
                  ? "Tanggal ini berada di luar masa batch magang."
                  : holidayLabel ?? "Akhir pekan — tidak ada jadwal absensi."}
              </p>
            </div>
          )}
        </div>
