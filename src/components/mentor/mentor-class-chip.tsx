...
import { GraduationCap, Layers, LayoutGrid, Users } from "lucide-react";
...
type Props = {
  /** Real assignment context (from the layout loader); null when unassigned. */
  context: MentorContext | null;
};

/**
 * Class chip — the Mentor counterpart to the Peserta-Magang InternshipClassChip.
 * Surfaces the class the mentor teaches (section letter at a glance, full
 * Batch/Bidang/Kelas context + mentee count in the popover). Matches the chip
 * ring/height of the other surfaces so it sits in the same visual family.
 */
export function MentorClassChip({ context }: Props) {
  if (!context) {
    return (
      <span
        className={cn(
  ...
  return (
    <Popover>
      <PopoverTrigger
        aria-label={`Kelas ${ctx.classFullName}, lihat detail bimbingan`}
        className={cn(
      ...
      >
        ...
        <dl className="space-y-3 px-5 pt-4 pb-5">
          ...
          <DetailRow
            ...
            value={`${ctx.studentCount} peserta`}
          />
        </dl>
