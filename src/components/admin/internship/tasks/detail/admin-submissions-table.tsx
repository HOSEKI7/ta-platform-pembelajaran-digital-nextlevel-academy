...
import { CheckCircle2, Download, RotateCcw, Users } from "lucide-react";

import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { TaskStatusBadge } from "@/components/internship/tasks/task-status-badge";
import type { AdminSubmissionRow } from "@/lib/admin-internship-tasks-query";

type Props = {
  rows: AdminSubmissionRow[];
  onForce: (row: AdminSubmissionRow) => void;
};
...
function PersonCell({ row }: { row: AdminSubmissionRow }) {
  return (
...
/** Download (if any) + force-toggle to the opposite stored status. */
function RowActions({
  row,
  onForce,
}: {
  ...
  const isSubmitted = row.rawStatus === "SUBMITTED";
  return (
...
/**
 * Class submission roster for a task. Unlike the mentor table (which "returns"
 * for revision), the admin action force-flips the stored submission status.
 */
export function AdminSubmissionsTable({ rows, onForce }: Props) {
  return (
      ...
      <div className="flex items-center gap-3 border-b border-zinc-100 px-5 py-4 dark:border-[color:var(--color-surface-border)] sm:px-6">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Ubah-paksa status pengumpulan peserta di kelas ini
          </p>
        </div>
      </div>
