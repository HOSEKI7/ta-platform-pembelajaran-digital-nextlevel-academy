...
import { Inbox, Loader2, Receipt, Search } from "lucide-react";
...
import { useAdminTransactionsQuery } from "@/hooks/use-admin-transactions";
...
export function AdminTransactionsView() {
  ...
  const params: AdminTransactionsParams = useMemo(
    () => ({
      page: urlPage,
      status: urlStatus,
      sort: urlSort,
  ...
  const txnQuery = useAdminTransactionsQuery(params);
  ...
  const updateUrl = useCallback(
    (
      ...
      if ("sort" in next) {
        if (next.sort && next.sort !== "date_desc") sp.set("sort", next.sort);
        else sp.delete("sort");
      }
      if ("page" in next) {
  ...
  const data = txnQuery.data;
  const isFetching = txnQuery.isFetching;
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader
        ...
        title="Riwayat"
        accent="Transaksi"
        description="Pantau seluruh transaksi pembelian kursus Peserta Didik — terima atau batalkan pembayaran tertunda, dan unduh bukti transaksi."
      />
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <SearchBox
          ...
          placeholder="Cari ID, pengguna, atau kursus…"
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            >
              <SelectValue>
                {(value: string) =>
                  SORT_LABELS[value as SortOption] ?? "Terbaru"
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
      ...
      <section className="overflow-hidden rounded-3xl bg-white ring-1 ring-zinc-200 dark:bg-[color:var(--color-surface-card)] dark:ring-[color:var(--color-surface-border)]">
        <div className="flex items-center justify-between gap-3 border-b border-zinc-100 px-5 py-4 dark:border-[color:var(--color-surface-border)] sm:px-6">
          <div className="flex items-center gap-3">
            <span className="grid size-10 place-items-center rounded-2xl bg-[color:var(--color-brand-50)] text-[color:var(--color-brand-700)] ring-1 ring-[color:var(--color-brand-100)] dark:bg-[color:var(--color-brand-500)]/15 dark:text-[color:var(--color-brand-200)] dark:ring-[color:var(--color-brand-500)]/30">
              <Receipt className="size-5" strokeWidth={2.2} />
            </span>
        ...
        {txnQuery.isPending ? (
          ...
          <ErrorState onRetry={() => txnQuery.refetch()} />
        ) : total === 0 ? (
          <EmptyState
            search={urlSearch}
            onReset={() => {
              setSearchInput("");
              updateUrl({ search: "", status: "all", sort: "date_desc" });
            }}
          />
        ) : (
          <TransactionsTable rows={data!.rows} isFetching={isFetching} />
        )}
      </section>
      ...
      {totalPages > 1 ? (
        <Pagination
          ...
          ariaLabel="Paginasi daftar transaksi"
        />
      ) : null}
    </div>
  );
...
function EmptyState({
  ...
  return (
    <div className="grid place-items-center gap-3 px-6 py-16 text-center">
      ...
      <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">
        {search.length > 0
          ? `Tidak ada transaksi dengan kata “${search}”`
          : "Belum ada transaksi untuk filter ini"}
      </p>
      <p className="max-w-md text-xs text-zinc-500 dark:text-zinc-300/70">
        Coba ubah kata kunci atau filter status — atau setel ulang filter.
      </p>
      <button
        type="button"
        onClick={onReset}
