...
import { useAdminCertificatesQuery } from "@/hooks/use-admin-certificates";
...
import { CertificateExpiryCard } from "./certificate-expiry-card";
import { CertificatesTable } from "./certificates-table";

type Props = {
  courseOptions: CertificateCourseOption[];
  expiry: CertificateExpirySetting;
};
...
const SORT_LABELS: Record<SortOption, string> = {
  issued_desc: "Terbit terbaru",
  issued_asc: "Terbit terlama",
};

const ALL_COURSES = "__all__";

export function AdminCertificatesView({ courseOptions, expiry }: Props) {
  ...
  const urlCourseId = parseCourseId(searchParams.get("courseId"));

  const [searchInput, setSearchInput] = useState(urlSearch);

  const courseTitleById = useMemo(() => {
    const map = new Map<string, string>();
    for (const c of courseOptions) map.set(c.id, c.title);
    return map;
  }, [courseOptions]);

  const params: AdminCertificatesParams = useMemo(
    () => ({
      page: urlPage,
      status: urlStatus,
      sort: urlSort,
    ...
    [urlPage, urlStatus, urlSort, urlSearch, urlCourseId],
  );

  const certificatesQuery = useAdminCertificatesQuery(params);
  ...
  const updateUrl = useCallback(
    (
      ...
      if ("sort" in next) {
        if (next.sort && next.sort !== "issued_desc") sp.set("sort", next.sort);
        else sp.delete("sort");
      }
      if ("courseId" in next) {
  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader
        ...
        title="Sertifikat"
        accent="Terbit"
        description="Pantau seluruh sertifikat yang telah diterbitkan — cari berdasarkan nomor, penerima, atau kursus, unduh ulang PDF, dan buka halaman verifikasi publik. Sertifikat yang sudah terbit tidak dapat dicabut."
      />

      <CertificateExpiryCard initialYears={expiry.years} />
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <SearchBox
          ...
          placeholder="Cari nomor, penerima, atau kursus…"
          ariaLabel="Cari sertifikat"
        />

        <div className="flex flex-wrap items-center gap-2">
          <Select
            value={urlCourseId || ALL_COURSES}
            onValueChange={(value) => {
              if (typeof value === "string")
                updateUrl({ courseId: value === ALL_COURSES ? "" : value });
            }}
          >
            <SelectTrigger
              className="h-10 min-w-48 max-w-64 rounded-full"
              aria-label="Filter kursus"
            >
              <SelectValue>
                {(value: string) =>
                  value === ALL_COURSES
                    ? "Semua Kursus"
                    : (courseTitleById.get(value) ?? "Semua Kursus")
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={ALL_COURSES}>Semua Kursus</SelectItem>
              {courseOptions.map((c) => (
          ...
          >
            ...
            >
              <SelectValue>
                {(value: string) =>
                  SORT_LABELS[value as SortOption] ?? "Terbit terbaru"
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
      ...
      <section className="overflow-hidden rounded-3xl bg-white ring-1 ring-zinc-200 dark:bg-[color:var(--color-surface-card)] dark:ring-[color:var(--color-surface-border)]">
        ...
        {certificatesQuery.isPending ? (
          ...
          <ErrorState onRetry={() => certificatesQuery.refetch()} />
        ) : total === 0 ? (
          <EmptyState
            search={urlSearch}
            onReset={() => {
          ...
          <CertificatesTable rows={data!.rows} isFetching={isFetching} />
        )}
      </section>
      ...
      {totalPages > 1 ? (
        <Pagination
          ...
          ariaLabel="Paginasi daftar sertifikat"
        />
      ) : null}
    </div>
  );
...
function EmptyState({
  ...
  return (
    <div className="grid place-items-center gap-3 px-6 py-16 text-center">
      ...
      <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">
        {search.length > 0
          ? `Tidak ada sertifikat cocok dengan “${search}”`
          : "Belum ada sertifikat untuk filter ini"}
      </p>
      <p className="max-w-md text-xs text-zinc-500 dark:text-zinc-300/70">
        Coba ubah kata kunci atau filter — sertifikat muncul di sini setelah
        peserta menyelesaikan kursus dan mengklaimnya.
      </p>
      <button
        type="button"
        onClick={onReset}
