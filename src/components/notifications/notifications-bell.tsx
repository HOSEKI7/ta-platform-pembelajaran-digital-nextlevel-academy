...
import type { NotificationDTO, NotificationsDTO } from "@/lib/student-data-loader";

type Props = {
  data: NotificationsDTO | undefined;
  isPending: boolean;
  isError: boolean;
  /**
   * Fired once each time the popover opens *while there are unread items* — the
   * caller wires this to its mark-all-read mutation. Kept as a callback (rather
   * than baking the mutation in) so this component stays role-agnostic and
   * purely presentational.
   */
  onOpenedWithUnread: () => void;
};

/**
 * Shared notifications bell — the single presentational source for the Peserta
 * Didik, Peserta Magang, and Mentor topbars. Each surface supplies its own
 * role-scoped query + mark-all-read mutation; the markup, numeric unread badge,
 * and "mark read on open" behaviour live here.
 */
export function NotificationsBell({
  ...
  // Mark everything read once on each open, but only when there are unread items.
  useEffect(() => {
    if (open && unread > 0) {
      onOpenedWithUnread();
    }
    // Intentionally keyed only on `open`: re-running after the optimistic update
    // flips unread to 0 would refire needlessly.
