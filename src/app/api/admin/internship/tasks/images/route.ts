...
/**
 * POST /api/admin/internship/tasks/images
 *
 * Upload a single inline image pasted/dropped into the task-description editor
 * on the admin Edit page. ADMINISTRATOR only (mirror of the mentor route).
 * Returns the stored Bunny object path + a freshly signed URL for display.
 */
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error("[admin/internship/tasks/images] upload failed", err);
    const msg =
