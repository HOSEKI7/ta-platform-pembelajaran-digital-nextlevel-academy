...
**Version:** 1.2.0  
**Status:** Final — Implementation Complete (v1.0 Full Release)  
...
├── /cert/:publicId (Verifikasi Sertifikat via URL publik — alternatif)
├── /undang-admin/terima (Terima undangan admin — publik, light mode)
...
├── /catalog (Katalog Kursus) -> **tidak ada menu navigasi di sidebar, hanya lewat hyperlink dari halaman dashboard dan halaman kursus saya**
├── /certificates (Sertifikat)
├── /exp-level (EXP & Level)
├── /transactions (Transaksi)
│   └── /transactions/:id (Detail Transaksi)
└── /settings (Profil & Keamanan)
```
...
│   ├── /admin/courses/new (Buat Course — 2-fase: form → draft → edit)
│   ├── /admin/courses/:id/edit (Edit Course + Kurikulum)
│   └── /admin/courses/categories (Kategori Course — CRUD modal)
├── /admin/users (Manajemen Pengguna)
│   ├── /admin/users/new (Tambah Pengguna)
│   └── /admin/users/:id/edit
├── /admin/admins (Akun Administrator — invite + manage)
├── /admin/certificates (Sertifikat — monitoring + konfigurasi expiry)
├── /admin/transactions (Transaksi — dengan aksi Terima/Batalkan)
├── /admin/vouchers
│   ├── /admin/vouchers/new (Buat Voucher — scope course/kategori + max 1 use)
│   └── /admin/vouchers/:voucherId/edit
├── /admin/gamification → redirect ke /admin/gamification/exp-rules
│   ├── /admin/gamification/exp-rules (Aturan EXP — read-only)
│   └── /admin/gamification/badges (Manajemen Badge — CRUD, modal dialog)
...
│   ├── /admin/internship/tasks (Monitoring lintas kelas)
│   │   └── /admin/internship/tasks/:taskId (Detail)
│   │       └── /admin/internship/tasks/:taskId/edit
│   ├── /admin/internship/grades (Nilai Akhir — override administratif)
│   ├── /admin/internship/config (Konfigurasi Magang — Batch/Bidang/Kelas)
│   └── /admin/internship/work-config (Konfigurasi Jam Kerja dan Libur — Holiday CRUD)
└── /admin/settings (Konfigurasi Platform — 4 tab: Profil/Keamanan/Informasi Platform/Status Integrasi)
```

> Seluruh halaman admin sudah dibangun dan aktif. Lihat §6.11.x untuk detail implementasi per halaman.
...
- Email: wajib, format valid, **unik di sistem** (dicocokkan apa adanya/lowercase — `user+tag@` dan `user@` dianggap berbeda).
- Password: wajib, min 8 karakter, harus mengandung huruf besar, huruf kecil, dan angka.

**Penolakan duplikat:** Pendaftaran dengan email yang sudah terdaftar **ditolak eksplisit** di halaman daftar dengan pesan "Email ini sudah terdaftar" (cek di `hooks.before` `/sign-up/email` → `USER_ALREADY_EXISTS`). Tidak ada email notifikasi terpisah.
...
- Update: username (unik), nama lengkap, email, foto profil (preset avatar saja — upload dihapus). **Peserta Magang dan Mentor terdapat Bidang dan Kelas, namun tidak dapat diubah. Administrator** dapat mengubah email (diverifikasi via tautan ke email baru). Mentor tidak dapat mengubah email (`lockEmail`).
- Jika email diubah: bila email tujuan sudah dipakai akun lain → ditolak `409 "Email sudah dipakai oleh akun lain."`; bila bebas → sistem mengirim **verifikasi ke email baru** sebelum perubahan disimpan (berlaku setelah tautan diklik). Endpoint ubah-email diberi rate-limit (5/jam/user). Pasca-verifikasi, email notifikasi "kini terdaftar" dikirim ke alamat baru.
...
- **Hero Section:** Headline utama, subheadline, CTA utama ("Lihat Kursus", "Mulai Belajar"). Parallax background layer (rAF-throttle, dimatikan di <768px & reduced-motion). Teks "Bersertifikat" dengan gradient animasi.
- **Brand Marquee:** CSS-only infinite scroll brand logo strip (pause on hover).
- **Highlight Fitur (Showcase):** Fitur unggulan platform (gamifikasi, sertifikat, magang) dengan scroll reveal animation.
- **Cara Kerja (How It Works):** 4-step horizontal rail dengan progress bar garis putus-putus.
- **Kursus Unggulan:** Tampilkan beberapa kursus terpopuler/terbaru (data dari backend).
- **Social Proof / Statistik:** Counter animasi (IntersectionObserver + rAF) untuk jumlah pelajar, kursus, sertifikat.
- **FAQ:** Accordion (base-ui) dengan scroll reveal.
- **CTA Section (FinalCTA):** Ajakan daftar akun dengan animasi bar "receipt".
- **Semua animasi scroll** memakai komponen `<Reveal>` (CSS + IntersectionObserver, tanpa library motion). `as` prop untuk polymorphic (div/article/li/figure). `prefers-reduced-motion` mematikan semua animasi.
- **Footer:** Dinamis — membaca Nama Platform dari `platform_setting` (`PLATFORM_INFO`). Fallback teks hardcoded bila kosong.
...
2. **Catatan** — catatan pribadi milik pengguna untuk tahap tersebut. Disimpan **per-user
   ...
   penyimpanan di-_debounce_ ~1,5 detik setelah pengguna berhenti mengetik (bukan tiap karakter,
   demi performa) dan di-_flush_ langsung saat pindah step / meninggalkan tab / menutup halaman.
...
| Tipe Diskon               | Enum (PERCENTAGE/FIXED) | Ya    | Persentase atau nominal rupiah                                                        |
| Persentase Diskon         | Integer (1–100)         | Ya\*  | Wajib jika tipe = PERCENTAGE; bisa 100%                                               |
| Nominal Diskon (Rp)       | Integer                 | Ya\*  | Wajib jika tipe = FIXED; potongan tetap dalam rupiah                                  |
| Tanggal Mulai Berlaku     | Date                    | Tidak | Kosong → aktif saat dibuat                                                            |
| Tanggal Kedaluwarsa       | Date                    | Ya    | Voucher tidak berlaku setelah tanggal ini                                             |
| Batas Penggunaan Total    | Integer                 | Tidak | Total berapa kali voucher bisa dipakai (null = unlimited)                             |
| Batas 1 Kali per Pengguna | Boolean                 | Tidak | Toggle: setiap user maksimal 1 kali pemakaian. Menerapkan state-machine lock (§6.8.3) |
| Berlaku untuk Kategori    | UUID (nullable)         | Tidak | Jika diisi, hanya kursus dgn kategori tsb yg bisa pakai voucher                       |
| Berlaku untuk Kursus      | Course ID (nullable)    | Tidak | Jika diisi, hanya kursus tersebut yg bisa pakai (null = semua kursus)                 |
...
\* Bergantung pada tipe diskon yang dipilih.

**Validasi saat user input voucher di checkout (updated):**

1. Kode ditemukan di database.
2. Status aktif = true.
3. Tanggal saat ini dalam rentang mulai–kedaluwarsa.
4. Belum mencapai batas penggunaan total.
5. **State-machine per-user lock** — jika `maxUsagePerUser` terisi (bukan null):
   - Order `PENDING` untuk voucher+user ini → lock active (1 slot terpakai sementara).
   - Order `SUCCESS` → exhausted (tidak bisa pakai lagi).
   - Order `FAILED` / `EXPIRED` / `CANCELED` → lock released (slot kosong, bisa dicoba lagi).
6. Role user sesuai ketentuan voucher.
7. User ID sesuai (jika ada ketentuan user ID).
8. Kategori kursus yang dibeli sesuai (jika ada ketentuan kategori).
...
#### 6.8.3 State-Machine Lock (Max 1 Kali per Pengguna)

Lock bersifat **universal** untuk semua voucher dengan `maxUsagePerUser != null`, termasuk voucher reward gamifikasi (`NLA-LV…`). Status pemakaian diturunkan dari status **Order** (tidak ada tabel lock terpisah):

| Kondisi Order                           | Status Lock   | Dampak                                   |
| --------------------------------------- | ------------- | ---------------------------------------- |
| Tidak ada order                         | Available     | Voucher bisa dipakai                     |
| Ada order `PENDING`                     | **Locked**    | Voucher tidak bisa dipakai (sementara)   |
| Ada order `SUCCESS`                     | **Exhausted** | Voucher tidak bisa dipakai (permanen)    |
| Ada order `FAILED`/`EXPIRED`/`CANCELED` | **Released**  | Voucher bisa dipakai lagi (retry sukses) |

> **Perbaikan bug:** Sebelumnya, reward voucher (`maxUsagePerUser: 1`) yang ordernya gagal tetap mengonsumsi slot secara permanen — user tidak bisa retry. Dengan state-machine lock, order gagal/batal melepaskan slot sehingga user bisa checkout ulang.

#### 6.8.4 Form Admin — Course Scope dengan Status Badge

Dropdown **"Berlaku untuk Kursus"** pada form Buat/Edit Voucher menampilkan seluruh course (termasuk DRAFT dan ARCHIVED) dengan **badge status** di samping judul:

- **DRAFT** → badge amber "Draft"
- **ARCHIVED** → badge zinc "Archived"
- **PUBLISHED** → tanpa badge (default)

Styling badge konsisten dengan `CourseStatusBadge` yang dipakai di tabel daftar course admin. Pemilihan course otomatis mengisi field kategori. Perubahan kategori manual akan mereset course (guard inkonsistensi di client + server).

**Tidak ada perubahan logika penyimpanan/validasi** — ini murni perubahan visual pada dropdown untuk membantu admin mengidentifikasi status course tanpa perlu membuka halaman terpisah.

#### 6.8.5 Edit Voucher — Tampilkan Scope yang Tersimpan

Halaman Edit Voucher memuat data `allowedCourseId` dan `allowedCategoryId` yang tersimpan ke dalam dropdown, sehingga admin dapat melihat dan mengubah scope voucher yang sudah ada.
...
3. Peserta Magang mendapat notifikasi in-app (`TASK_FEEDBACK`).
...
| Penerima       | Trigger                                                                                                          | Target Navigasi (saat diklik) |
| -------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| Peserta Didik  | Pembelian course berhasil & course dimiliki (`PURCHASE_SUCCESS`) — ucapan selamat + ajakan mulai belajar         | `/transactions/[refId]`       |
| Peserta Didik  | Progres course mencapai 100% & sertifikat siap diklaim (`CERTIFICATE_READY`) — ucapan selamat + ajakan klaim     | `/certificates`               |
| Peserta Magang | Mendapat tugas baru dari mentor (`TASK_ASSIGNED`)                                                                | `/internship/tasks/[refId]`   |
| Peserta Magang | Tugas mendapat feedback (dikembalikan) dari mentor (`TASK_FEEDBACK`) — ajakan baca feedback                      | `/internship/tasks/[refId]`   |
| Peserta Magang | Nilai akhir magang diisi/diubah oleh mentor (`FINAL_GRADE_POSTED`)                                               | `/internship/final-grade`     |
| Mentor         | Peserta magang mengumpulkan tugas (`TASK_SUBMITTED`)                                                             | `/mentor/tasks/[refId]`       |
| Mentor         | Admin mengubah nilai akhir peserta bimbingannya (`FINAL_GRADE_OVERRIDE`) — pesan memuat perubahan nilai + alasan | `/mentor/grades`              |

> Target navigasi ditentukan **client-side** dari `type` + `refId` via helper `notificationHref()` di `src/lib/notification-href.ts`. Tipe tak dikenal → row non-klik.
...
| --------------------------------------- | ------------- | ------------------------------------------------------------------ |
| Verifikasi email (registrasi)           | User baru     | Link verifikasi email                                              |
| Lupa password                           | User          | Link reset password (valid 1 jam)                                  |
| Perubahan password                      | User          | Notifikasi password berhasil diubah                                |
| Perubahan email                         | User          | Link verifikasi email baru + notif "kini terdaftar" ke alamat baru |
...
| Certificate ready (`CERTIFICATE_READY`) | Peserta Didik | Pengingat untuk klaim sertifikat                                   |
| Undangan admin baru                     | Calon admin   | Link accept undangan (valid 24 jam)                                |
...
#### 6.11.12 Pengelolaan Akun Administrator _(implemented)_

`ADMINISTRATOR` adalah role tertinggi. Karena UI Manajemen Pengguna (§6.11.4) **sengaja tidak** dapat membuat role Administrator, penambahan administrator ditangani oleh alur khusus berikut.

**a. Bootstrap administrator pertama.** Script idempotent `scripts/bootstrap-admin.ts` (`npm run bootstrap:admin`) membaca kredensial dari environment (`BOOTSTRAP_ADMIN_EMAIL`, `BOOTSTRAP_ADMIN_NAME`, `BOOTSTRAP_ADMIN_PASSWORD`), membuat `User`(ADMINISTRATOR) + `Account` credential, dan menandai `mustChangePassword=true` sehingga admin **dipaksa mengganti password saat login pertama** (guard ditambahkan di `(admin)/layout.tsx` → redirect `/ganti-password`). Aman dijalankan berulang; bila akun sudah ada, script mempromosikan/memreparasi (mis. akun lama tanpa credential).

**b. Undang administrator baru (email, sekali pakai, 24 jam).** Halaman `/admin/admins` ("Akun Admin") menyediakan tombol **Undang Admin** (email + nama opsional). Sistem membuat baris `AdminInvite` (hanya **hash SHA-256** token yang disimpan; token plaintext hanya ada di tautan email), mengirim email undangan (template `admin-invite`), dan **tidak** membuat `User` sampai undangan diterima. Penerima membuka halaman publik `/undang-admin/terima?token=…`, **menetapkan nama + password sendiri**, lalu akun `User`(ADMINISTRATOR, terverifikasi, aktif) dibuat. Token kedaluwarsa **24 jam** (env `ADMIN_INVITE_EXPIRY_HOURS`, default 24), sekali pakai, dan dapat **dibatalkan** selama masih pending. Saat email gateway (Resend) tidak dikonfigurasi, tautan undangan dikembalikan ke admin pengundang (dan dicetak ke log) agar dapat dikirim manual.

**c. Kelola akun administrator.** Di `/admin/admins`, **setiap** administrator (semua peer setara) dapat: melihat daftar admin, **menonaktifkan/mengaktifkan** admin lain, **menghapus** (soft delete) admin lain, dan **membatalkan** undangan yang masih pending.

**Aturan yang ditegakkan server-side:**

- Sistem **tidak boleh memiliki 0 administrator aktif** — aksi nonaktif/hapus yang akan menyisakan nol admin aktif ditolak (409).
- Administrator **tidak bisa menonaktifkan/menghapus akunnya sendiri** (403).
- Sesi user **dicabut** saat dinonaktifkan/dihapus (login juga diblok oleh database hook `isActive`).
- Tanpa batas jumlah admin, namun bila administrator aktif **lebih dari 5**, ditampilkan **peringatan non-blocking** soal risiko keamanan, privasi, dan kontrol data berlebih.
- Setiap perubahan dicatat di `AuditLog` (`BOOTSTRAP_ADMIN`, `ADMIN_INVITE_CREATE/ACCEPT/REVOKE`, `ADMIN_STATUS_CHANGE`, `ADMIN_DELETE`).

**Desain otorisasi (extensible).** Kapabilitas administrator dipusatkan di `src/lib/admin-permissions.ts` (mis. `canManageAdmins(role)`, `ADMIN_ROLES`) — bukan pengecekan `role === ADMINISTRATOR` yang tersebar. Untuk menambah tingkat admin yang lebih rendah di masa depan cukup menambah role ke enum + modul ini; titik pemanggil tidak perlu berubah.
...
| **Bundler**              | Turbopack                | Development server                                                                                                         |
| **Styling**              | Tailwind CSS + shadcn/ui | v4 (PostCSS plugin, @theme di globals.css)                                                                                 |
| **UI Components**        | base-ui + shadcn/ui      | Komponen presentasi + primitif                                                                                             |
...
| **ORM**                  | Prisma 7                 | Schema-first, @prisma/adapter-pg                                                                                           |
| **Authentication**       | Better Auth              | Sessions, email-password, email verifikasi                                                                                 |
| **Video & File Hosting** | Bunny.net                | Stream (video, signed embed) + Storage (task attachments, thumbnails, ikon, gambar Tiptap) + Certificates zone (tokenless) |
| **Payment Gateway**      | Midtrans                 | Snap popup (inline), webhook-driven                                                                                        |
| **Email**                | Resend + React Email     | Transactional email, tanpa queue                                                                                           |
| **Input Validation**     | Zod                      | Frontend & Backend (Route Handlers)                                                                                        |
| **Certificate Render**   | Satori + Sharp + pdf-lib | PNG source → PDF wrapper; bukan @react-pdf                                                                                 |
| **Testing**              | Playwright (terinstal)   | E2E (belum ada test suite)                                                                                                 |
| **Deployment**           | VPS (standalone build)   | Build di CI, artifact di-copy ke VPS                                                                                       |
...
| Prisma 7 + @prisma/adapter-pg      | Driver adapter menghilangkan bundled query engine; `prisma.config.ts` untuk mode session pooler        |
| Next.js 16 proxy.ts                | Next.js 16 rename `middleware.ts` → `proxy.ts` untuk Edge; role enforcement tetap di Server Components |
| Custom theme provider              | Tidak pakai `next-themes` (React 19.2 warning); custom `ThemeProvider` di `src/components/providers/`  |

---

## 9. API Structure Overview

Seluruh API endpoint menggunakan prefix `/api/` (tanpa `/v1/`), diimplementasikan sebagai Next.js Route Handlers di `src/app/api/`. Arsitektur REST (bukan tRPC/GraphQL).

### 9.1 Auth (Better Auth — prefix `/api/auth/`)

| Method | Endpoint                    | Deskripsi                  | Auth   |
| ------ | --------------------------- | -------------------------- | ------ |
| POST   | `/api/auth/sign-up/email`   | Registrasi peserta didik   | Public |
| POST   | `/api/auth/sign-in/email`   | Login email+password       | Public |
...
| POST   | `/api/auth/change-password` | Ganti password (auth)      | Auth   |
| GET    | `/api/auth/verify-email`    | Verifikasi email via token | Public |
...
| GET    | `/cert/:publicId`                   | Verifikasi publik                     | Public |
| GET    | `/api/public/cert/:publicId/image`  | Lazy-regen fallback gambar sertifikat | Public |
...
### 9.8 Internship — Mentor

| Method | Endpoint                                          | Deskripsi                 | Auth   |
| ------ | ------------------------------------------------- | ------------------------- | ------ |
| GET    | `/mentor/dashboard`                               | Data dashboard mentor     | MENTOR |
| GET    | `/mentor/students`                                | Daftar peserta bimbingan  | MENTOR |
| POST   | `/mentor/attendance/check-in`                     | Check-in absensi pribadi  | MENTOR |
| GET    | `/mentor/attendance`                              | Rekap absensi pribadi     | MENTOR |
| GET    | `/mentor/student-attendance`                      | Rekap absensi peserta     | MENTOR |
| GET    | `/mentor/tasks`                                   | Daftar tugas yang dibuat  | MENTOR |
| POST   | `/mentor/tasks`                                   | Buat tugas baru           | MENTOR |
| GET    | `/mentor/tasks/:id`                               | Detail tugas              | MENTOR |
| PUT    | `/mentor/tasks/:id`                               | Edit tugas                | MENTOR |
| DELETE | `/mentor/tasks/:id`                               | Hapus tugas               | MENTOR |
...
| GET    | `/mentor/notifications`                           | Notifikasi in-app mentor  | MENTOR |
| POST   | `/mentor/notifications/mark-all-read`             | Tandai semua terbaca      | MENTOR |
...
| CRUD         | `/admin/courses/categories`               | Kelola kategori              | ADMIN |
| CRUD         | `/admin/courses/:id/sprints`              | Kelola sprint                | ADMIN |
| CRUD         | `/admin/sprints/:id/steps`                | Kelola tahap                 | ADMIN |
| PATCH        | `/admin/courses/:courseId/status`         | Ubah status course           | ADMIN |
| CRUD         | `/admin/users`                            | Kelola pengguna              | ADMIN |
| GET/POST/PUT | `/admin/vouchers` + `/:id`                | Kelola voucher (CRUD penuh)  | ADMIN |
| GET/PATCH    | `/admin/transactions` + `/:id`            | Kelola transaksi             | ADMIN |
| POST         | `/admin/transactions/:id/accept`          | Terima pembayaran            | ADMIN |
| POST         | `/admin/transactions/:id/cancel`          | Batalkan pembayaran          | ADMIN |
| DELETE       | `/admin/transactions/:id`                 | Hapus transaksi (soft)       | ADMIN |
| GET          | `/admin/certificates`                     | Monitor sertifikat           | ADMIN |
| GET/POST     | `/admin/gamification/exp-rules`           | Aturan EXP (read-only)       | ADMIN |
| CRUD         | `/admin/gamification/badges`              | Kelola badge                 | ADMIN |
| GET/PATCH    | `/admin/internship/mentor-attendance`     | Rekap & edit absensi mentor  | ADMIN |
| GET/PATCH    | `/admin/internship/student-attendance`    | Rekap & edit absensi peserta | ADMIN |
| CRUD         | `/admin/internship/config/batches`        | Kelola batch                 | ADMIN |
| CRUD         | `/admin/internship/config/fields`         | Kelola bidang                | ADMIN |
| CRUD         | `/admin/internship/config/classes`        | Kelola kelas                 | ADMIN |
| CRUD         | `/admin/internship/work-config/holidays`  | Kelola tanggal libur         | ADMIN |
| GET/DELETE   | `/admin/internship/tasks` + `/:id`        | Monitor tugas                | ADMIN |
| PUT          | `/admin/internship/tasks/:id`             | Edit tugas                   | ADMIN |
| POST         | `/admin/internship/tasks/:id/submissions` | Koreksi status pengumpulan   | ADMIN |
| GET/POST/PUT | `/admin/internship/grades`                | Monitor & edit nilai akhir   | ADMIN |
| GET          | `/admin/admins`                           | Daftar akun admin            | ADMIN |
| POST         | `/admin/admins/invite`                    | Undang admin baru            | ADMIN |
| POST         | `/admin/admins/:id/toggle-active`         | Nonaktifkan/aktifkan admin   | ADMIN |
| DELETE       | `/admin/admins/:id`                       | Hapus admin (soft)           | ADMIN |
| POST         | `/admin/notifications/indicators/seen`    | Tandai indikator sidebar     | ADMIN |
| GET          | `/admin/settings`                         | Konfigurasi platform         | ADMIN |
| PATCH        | `/admin/settings/platform-info`           | Update informasi platform    | ADMIN |
| GET          | `/admin/settings/integration-status`      | Cek integrasi eksternal      | ADMIN |
...
**Auth & endpoint umum** — ditangani built-in rate limiter Better Auth (storage Redis bila dikonfigurasi, fallback in-memory), key per IP:
...
| POST `/*/vouchers/validate`      | 30 per menit / user | Brute-forceable (enumerasi kode voucher)                        |
| POST `/api/account/email-change` | 5 per jam / user    | Cegah spam permintaan ubah-email + email verifikasi             |
| POST `/api/payment/webhook`      | Tidak di-rate-limit | Otentik via signature SHA512                                    |

- **Backend store:** Redis **self-hosted** (bukan layanan cloud), diaktifkan via env `RATE_LIMIT_REDIS_URL`. Bila kosong → fallback **in-memory** otomatis (clone-and-run dev tanpa setup Redis; tetap berfungsi di VPS karena proses Node persisten). Koneksi Redis tunggal reusable (`ioredis`). Di dev, `RATE_LIMIT_REDIS_URL` dikosongkan untuk menghindari error ECONNREFUSED (Redis tidak jalan). Produksi/uji: `docker compose up -d redis` + isi URL.
- **Limiter auth Better Auth** dibacking Redis bersama via `rateLimit.customStorage` → window tahan restart/deploy. **Degradasi (BUKAN fail-open):** bila Redis mati, limiter auth turun ke jaring in-memory **ketat** dengan kuota yang sama — tidak pernah melepas limit.
- **Fail-open (khusus endpoint pembelian):** `rate-limiter-flexible` untuk endpoint pembelian tetap **fail-open** by design: masalah infra tak boleh memblokir pembelian sah. Bila Redis tidak terjangkau, request **tetap diizinkan**.
- **Lapisan tambahan**, tidak menggantikan proteksi existing (sesi, advisory lock per `(user,course)`, guard double-purchase, state-machine voucher lock).
- **Operasional:** `docker-compose.yml` menyediakan Redis (bind `127.0.0.1`, password wajib, persistence mati).

### 10.4 Video Protection (Bunny.net)
...
- **Breakpoints:** sm (640px), md (768px), lg (1024px), xl (1180px).
...
| Email Berubah            | "Email Anda Telah Diubah"                    | Notifikasi informatif ke alamat baru                                                 |
| Konfirmasi Checkout      | "Menunggu Pembayaran — [Nama Kursus]"        | Ringkasan order + jumlah bayar + instruksi + tombol Lanjutkan Pembayaran + countdown |
...
| Undangan Admin Baru      | "Undangan Admin — NextLevel Academy"         | Link accept (valid 24 jam) + instruksi                                               |

---

## 13. Constraints & Decisions Log
...
| 19  | Sertifikat: Satori+Sharp PNG (bukan @react-pdf)         | Satu sumber desain (JSX→PNG); PDF hanya wrapper pdf-lib. Headless browser tidak dipakai                           |
| 20  | Bunny Storage untuk file tugas/thumbnail/ikon           | Supabase free tier 1GB tidak cukup; Bunny Storage lebih murah, signed URL untuk akses                             |
| 21  | Avatar preset WebP (bukan upload ke Supabase)           | File kecil/statis di repo `public/`, nol biaya CDN, version-controlled, tidak perlu storage eksternal             |
| 22  | maxUsagePerUser nullable = unlimited (bukan default 1)  | Perubahan skema: null = no cap, 1 = sekali per user. Toggle Boolean di form admin                                 |
| 23  | State-machine lock per-user (derived from Order.status) | Tidak ada tabel lock terpisah. PENDING=kunci, FAILED/EXPIRED/CANCELED=bebas. Memperbaiki bug retry reward voucher |
| 24  | Voucher: tipe diskon PERCENTAGE + FIXED                 | Admin dapat memilih persen atau nominal rupiah; helper bersama `computeVoucherDiscount`                           |
| 25  | Halaman admin invite Bootstrap (`/admin/admins`)        | Role ADMINISTRATOR dibuat via undangan email (24 jam), bukan via form pengguna biasa                              |
| 26  | Next.js 16 + Turbopack                                  | Perubahan major dari Next.js 14; file middleware → proxy.ts; tanpa next-themes, pakai custom ThemeProvider        |
| 27  | Prisma 7 + @prisma/adapter-pg                           | Driver adapter menggantikan bundled query engine; `prisma.config.ts` opsional untuk koneksi                       |
...
| Promosi setting window absen ke tabel database    | Masih konstanta global di `internship-config.ts` |
